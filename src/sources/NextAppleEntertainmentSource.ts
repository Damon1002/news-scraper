...
export class NextAppleEntertainmentSource extends NewsSource {
  ...
  private async parseArticlesWithDates(html: string, category: NewsCategory): Promise<NewsItem[]> {
    ...
    $('a[href*="/entertainment/"]').each((index, element) => {
      ...
      // Build full URL
      const fullUrl = href.startsWith('http') ? href : `https://tw.nextapple.com${href}`;
      ...
      if ($img.length) {
        ...
        if (imgSrc) {
          imageUrl = imgSrc.startsWith('http') ? imgSrc : `https://tw.nextapple.com${imgSrc}`;
        }
      }
  ...
  private async fetchArticleDate(articleUrl: string): Promise<Date> {
    try {
      ...
      if (timeElement.length) {
        ...
        if (timeText) {
          // Convert format "2025/07/13 16:41" to ISO format
      ...
      // Priority 4: Try to extract from article date patterns in content
      const datePattern = /(\d{4})年(\d{1,2})月(\d{1,2})日\s+(\d{1,2}):(\d{2})/;
      const bodyText = $('article, .article-content, .content').text();
      ...
      // Priority 5: Try to extract from URL date pattern (YYYYMMDD format)
      const urlDateMatch = articleUrl.match(/\/(\d{8})\//);
      if (urlDateMatch) {
        const [, dateStr] = urlDateMatch;
        const year = dateStr.substring(0, 4);
        const month = dateStr.substring(4, 6);
        const day = dateStr.substring(6, 8);
        const taiwanTime = `${year}-${month}-${day}T12:00:00+08:00`;
